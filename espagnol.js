    // ------------------------------------------------------------------
    // 1. DÉFINITION DES DONNÉES (Gardez le même bloc dans les deux fichiers)
    // ------------------------------------------------------------------
    const videoData = {
        '1': {
            title: 'El Fotón: ¿Partícula u Onda?',
            channelName: 'Henidos',
            subscribers: '1M',
            views: '1.9M',
            age: '7 days ago',
            description: `¡Hola a todos! 👋 En esta primera parte de nuestra serie de física, vamos a descolgar el teléfono para hablar de los bloques de construcción más pequeños del universo: ¡los fotones!

    ¿Sabías que la luz se comporta de dos maneras a la vez? Exploramos la base de la física cuántica:
    • ¿Qué son las **partículas** (electrones, protones)?
    • ¿Qué son las **ondas** mecánicas?
    • La **dualidad onda-partícula** (el concepto clave para entenderlo todo).

    Descubre cómo los fotones desafían nuestra lógica. ¡Es la introducción perfecta al mundo de lo muy pequeño!

    ---
    🔗 **¡Mira la Parte 2 (El Experimento de la Doble Rendija) aquí!**
    ---
    ...
    **#FísicaCuántica #Fotones #Partículas #Ondas #DualidadOndaPartícula #Ciencia #DivulgaciónCientífica #Yann**`,
        ...
        '2': {
            title: '¿Por Qué la Observación lo Cambia Todo? (Doble Rendija)',
            channelName: 'DosMinutos',
            subscribers: '8M',
            views: '7M',
            age: '6 day ago',
            description: `¡Hola a todos y bienvenidos a la Parte 2! 👋 Soy Yann. En este vídeo, vamos a sumergirnos en el experimento más famoso, fascinante y misterioso de la física cuántica: El Experimento de la Doble Rendija (Double Slit Experiment).

    Después de ver qué son las partículas y las ondas en la Parte 1, hoy descubrimos por qué los electrones se comportan de forma totalmente ilógica cuando nadie los está mirando. ¡Es la clave de la dualidad onda-partícula!

    🚀 En este vídeo aprenderás:
    • ¿Qué es el experimento de la doble rendija?
    • ¿Por qué los electrones actúan como ondas?
    • El misterioso efecto de la observación en el mundo cuántico.

    ---
    🔗 *Mira la Parte 1 (Introducción a la Cuántica) aquí:*
    ...
    **#DobleRendija #FísicaCuántica #Ciencia #DivulgaciónCientífica #MecánicaCuántica #DualidadOndaPartícula #Yann**`,
        ...
        'playlist': {
            title: 'playlist avec les 2 vidéo',
            channelName: 'Henidos & DosMinutos',
            subscribers: '1.9M',
            views: 'Updated today',
            description: 'Ceci est une playlist contenant les deux vidéos.',
            thumbnailSrc: 'espagnol.png',
            isPlaylist: true,
            videos: [
                { id: '1', title: 'El Fotón: ¿Partícula u Onda?', duration: '2:00', channel: 'Henidos' },
                { id: '2', title: '¿Por Qué la Observación lo Cambia Todo? (Doble Rendija)', duration: '2:00', channel: 'DosMinutos' }
            ]
        }
    };
    
    // ------------------------------------------------------------------
    // 2. LOGIQUE DU MENU BURGER (À conserver)
    // ------------------------------------------------------------------
    ...
    function toggleSidebar() {
        ...
        // ... (La logique du toggle est la même) ...
        if (isExpanded) {
            const width = '0px';
    ...
    // Fonction utilitaire pour générer le HTML d'une carte de suggestion
    ...
    const viewsLine = isPlaylist ? 'Updated today' : `${views} • ${age}`;
    ...
    // Fonction qui rend les suggestions dans la colonne latérale (Exclut la vidéo courante)
    ...
    // 2. Ajouter les autres vidéos pertinentes
    ...
    function loadVideoContent(contentId, videoInPlaylistId = null) {
        ...
        let videoIdForSuggestions = contentId; // ID utilisé pour le filtrage

        // 1. Logique spécifique à la Playlist
        if (contentData.isPlaylist) {
            ...
            // Remplir le conteneur de playlist sur le côté
            let playlistHTML = `
            ...
            // 2. Logique pour une vidéo normale
            playlistContainer.style.display = 'none';
            if (suggestedVideosWrapper) suggestedVideosWrapper.style.display = 'block';
            
            // Rendre les suggestions basées sur l'ID de la vidéo principale
            renderSuggestions(videoIdForSuggestions);
        }

        // 3. Mise à jour des éléments HTML du lecteur (LA SOLUTION DYNAMIQUE !)
        ...
        if (videoElement && videoDataToDisplay.videoSrc) {
            // Si la balise video est présente et a une source
            videoElement.src = videoDataToDisplay.videoSrc;
            videoElement.load(); // Nécessaire pour charger la nouvelle source
            videoElement.play(); // Démarrer la lecture automatiquement
            ...
            playerPlaceholder.textContent = `LECTEUR VIDÉO : ${videoDataToDisplay.title}`;
        }

        // 4. Mise à jour des éléments HTML du lecteur
        ...
        document.querySelector('.player-placeholder').textContent = `LECTEUR VIDÉO : ${videoDataToDisplay.title}`;
        document.querySelector('.description-box .views-date').innerHTML = 
            `${videoDataToDisplay.views} <span class="dot-separator">•</span> ${videoDataToDisplay.age}`;
    ...
    // ------------------------------------------------------------------
    // 4. EXÉCUTION & GESTION DES BOUTONS/LIENS
    // ------------------------------------------------------------------
    ...
    if (document.title.includes('Titre de la Vidéo')) {
        ...
        // --- Boutons Like/Dislike (Logique simplifiée) ---
